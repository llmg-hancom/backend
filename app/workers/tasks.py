...
def celery_beat_test():
    ...
    process_and_insert_to_db(DB_HOST, DB_PORT, OLLAMA_PORT, TARGET_DIR, "__korean_precedents.json")
...
def split_markdown_chunks_with_fallback(
    text: str, max_chunk_size: int = 500, chunk_overlap: int = 50,
    ...
    for doc in first_split_documents:
        ...
        if len(content) > max_chunk_size:
            ...
            for sub_doc in second_split_documents:
                ...
                final_chunks.append({"page_content": sub_doc.page_content, "metadata": sub_doc.metadata})
        else:
            final_chunks.append({"page_content": content, "metadata": metadata})
  
    return final_chunks
def conn_embedding_model(db_host, db_port, ollama_port):
    """
    DB 및 임베딩 모델 연결을 초기화합니다.
    """
    # 2. 임베딩 클라이언트 초기화 
    # ❗️ Ollama 접속 주소를 전달받은 db_host와 ollama_port로 설정
    OLLAMA_BASE_URL = f"http://{db_host}:{ollama_port}/"
    ...
    print(f"[INFO] Ollama 임베딩 모델 '{OLLAMA_MODEL}' 초기화 완료. (URL: {OLLAMA_BASE_URL})")
    ...
    try:
        ...
        print(f"[심각 오류] 데이터 삽입을 위한 DB 연결 실패: {e}")
...
def process_and_insert_to_db(
    db_host, db_port, ollama_port, target_directory, file_name
    ...
    try:
        ...
        print(f"[INFO] 총 {len(data_list)}개 항목 로드 완료.")
    except Exception as e:
    ...
    embeddings, conn, cur = conn_embedding_model(db_host, db_port, ollama_port)
    ...
    for item_data in tqdm(data_list, desc="Processing and Inserting"):
        ...
        try:
            ...
            file_name = item_data.get('사건번호', f"id_{item_data.get('판례정보일련번호', 'unknown')}")
            ...
            if not full_text:
                print(f"\n[WARNING] '{text_key}' 키가 없어 항목을 건너뜁니다.")
                conn.rollback() 
                continue 

            # 2. 텍스트 전처리 및 분할 실행
            markdown_formatted_text = preprocess_text_to_markdown(full_text, JUDGMENT_HEADER_KEYS)
            ...
            for chunk in chunks:
                ...
                cur.execute(insert_chunk_query, (document_id, chunk_content, vector_data, metadata_json))
            ...
            print(f"\n[오류] 항목 처리 및 DB 삽입 실패 (사건번호: {item_data.get('사건번호', 'N/A')}) 상세: {e}")
