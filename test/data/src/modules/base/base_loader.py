...
API_CONFIGS = {
    ...
    # 6. 조약 
    "trty":{
        "list_root":"TrtySearch",
        "data_root" :"Trty",
        "detail_root":"MultTrtyService",
        "filename_prefix": "trty",
        "detail_param": "ID",
        "serial_key":"조약일련번호",
        "friendly_name":"조약"
    }
...
class BaseLoader(ABC):
    ...
    def __init__(self, type, db_local_port, ollama_local_port,  markdown_splitter, recursive_splitter):
        
        # 인스턴스화된 실제 클래스 이름(PrecedentDownloader)으로 로거 생성
        ...
        self.db_local_port = db_local_port
        ...
        config = API_CONFIGS[type]
        self.friendly_name = config['friendly_name']


        
    def _init_embeddings(self) -> OllamaEmbeddings:
        # Ollama 임베딩 클라이언트를 초기화합니다. 
        OLLAMA_TUNNEL_URL = f"http://{self.LOCAL_HOST}:{self.ollama_local_port}/"
        embeddings = OllamaEmbeddings(model=self.OLLAMA_MODEL, base_url=OLLAMA_TUNNEL_URL)
        self.logger.info(f"Ollama 임베딩 모델 '{self.OLLAMA_MODEL}' 초기화 완료.")
        return embeddings
    ...
    def connect_db(self) -> psycopg.Connection | None:
        ...
        DB_CONFIG={
            ...
            'host': self.LOCAL_HOST, 'port': self.db_local_port
        }
        try:
            conn = psycopg.connect(**DB_CONFIG)
            register_vector(conn)
            self.logger.info("PostgreSQL DB 연결 성공.")
            return conn
        except Exception as e:
            self.logger.error(f"[심각 오류] DB 연결 실패: {e}")
            return None
    ...
    def _prepare_document(self, data: Dict[str, Any]) -> Dict[str, Any] | None:
        self.logger.info(f"Received Data Sample: {list(data.keys())[:10]}")
            ...
            for doc in tqdm(documents_to_insert, desc="Processing Documents"):
                try:
                    ...
                    if existing_result:
                        ...
                        self.logger.info(f"기존 Document ID {existing_doc_id} ('{doc['file_name']}') 삭제 완료 (file_path 중복).")
                    ...
                    if not inserted_result:
                        ...
                        self.logger.error(f"Document 삽입 실패: {doc['file_name']}")
                        conn.rollback() 
                        continue

                    document_id = inserted_result[0]
                    self.logger.debug(f"Document ID {document_id} 삽입 완료. 청크 분할 시작.")
                    ...
                    if all_chunks_for_doc:
                        ...
                        self.logger.info(f"Document ID {document_id}에 대해 총 {count_current_doc}개의 청크를 일괄 삽입합니다.")
                        
                        # ⭐️⭐️⭐️ [수정된 부분] psycopg 3 표준 COPY 방식 ⭐️⭐️⭐️
                        # 별도의 import 없이 cursor 객체의 copy 메서드를 사용합니다.
                        with cur.copy("COPY document_chunks (document_id, content, embedding, meta) FROM STDIN") as copy:
                        ...
                        self.logger.info(f"✅ Document ID {document_id} 및 청크 삽입/커밋 완료.")
                    else:
                        self.logger.warning(f"Document ID {document_id}의 청크 데이터가 없어 document_chunks 삽입을 건너뜁니다.")
                        conn.commit() 
                        processed_doc_count += 1

                except Exception as doc_error:
                    self.logger.error(f"Document '{doc.get('file_name', 'Unknown')}' 처리 중 오류 발생: {doc_error}. 해당 문서는 롤백됩니다.")
                    conn.rollback() # 현재 문서에 대한 모든 작업 롤백
                    continue # 다음 문서로 이동
            
            self.logger.info(f"=== ETL 파이프라인 최종 요약: {processed_doc_count}개의 {self.friendly_name} 문서 처리 완료, 총 {inserted_chunk_count}개의 청크 삽입/갱신 완료. ===")
            cur.close()

        except Exception as e:
            self.logger.error(f"FATAL ETL ERROR (전체 파이프라인): {e}")
