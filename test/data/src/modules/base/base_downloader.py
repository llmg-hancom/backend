...
class BaseDownloader(ABC):
    ...
    def _request_api(self, url: str, params: Dict[str, Any]) -> Dict[str, Any] | None:
        ...
        try:
            ...
            self.logger.error(f"❌ HTTP 오류 {e.response.status_code} 발생: {e}")
            return None
        except requests.exceptions.RequestException as e:
            self.logger.error(f"❌ 요청 오류 발생: {e}")
            return None
        except json.JSONDecodeError:
            self.logger.error(f"❌ JSON 디코딩 오류: 응답이 JSON 형식이 아닙니다.")
            return None
    ...
    def request_list_and_save(self) -> str | None:
        ...
        while page_no <= total_pages:
            ...
            if not data or list_root_key not in data:
                self.logger.error(f"❌ 목록 조회 실패 (페이지 {page_no}). 파이프라인 중단.")
                break
            ...
            if page_no == 1:
                ...
                if total_count == 0:
                    self.logger.info(f"❌ 조회된 {self.FRIENDLY_NAME} 가 없습니다.")
                    return None
                    
                total_pages = (total_count + self.ROWS_PER_PAGE - 1) // self.ROWS_PER_PAGE
                self.logger.info(f"총 {total_count}건, {total_pages} 페이지를 발견했습니다.")

            if isinstance(item_list, list) and item_list:
                all_items.extend(item_list)
                self.logger.info(f"페이지 {page_no}/{total_pages} 로드 완료. 현재까지 {len(all_items)}건 수집.")
            elif page_no > 1 and not item_list:
                self.logger.info(f"페이지 {page_no}에서 항목이 없어 조기 종료합니다.")
                break
        ...
        try:
            ...
            self.logger.info(f"🎉 전체 {self.FRIENDLY_NAME} 목록 데이터 ({len(all_items)}건) 저장 성공! 파일명: {self.list_output_path}")
            return self.list_output_path
        except Exception as e:
            self.logger.error(f"❌ 최종 파일 저장 중 오류 발생: {e}")
            return None
        pass
    @abstractmethod
    def request_detail_and_save(self, list_file_path: str) -> str | None:
        ...
        try:
            ...
            self.logger.error(f"❌ {self.FRIENDLY_NAME} 목록 파일 로드 실패: {list_file_path} - {e}")
            return None
        ...
        if pending_count == 0:
            self.logger.info("🎉 모든 상세 정보가 이미 기존 파일에 존재합니다. 작업을 종료합니다.")
        ...
        for item in tqdm(pending_items, desc="Requesting Remaining Details"):
            ...
            if detail_data and detail_root_key in detail_data:
                ...
                if detail_item and isinstance(detail_item, dict):
                    ...
                    self.logger.info(f"현재까지 {len(existing_precedents)}개 데이터가 추가되었습니다.")
                # ------------------------------------------------------------------
                ...
                if len(existing_precedents) % SAVE_INTERVAL == 0:
                    self.logger.info(f"   [Checkpoint] {len(existing_precedents)}건 저장 중... (파일 덮어쓰기)")
                    try:
                        with open(self.detail_output_path, 'w', encoding='utf-8') as f:
                            json.dump(existing_precedents, f, indent=4, ensure_ascii=False)
                    except Exception as e:
                        self.logger.error(f"❌ {self.FRIENDLY_NAME} ID {prec_id}: 중간 파일 저장 중 오류 발생 - {e}")
    ...
    def _load_existing_data(self) -> Tuple[List[Dict[str, Any]], Set[str]]:
        ...
        if os.path.exists(self.detail_output_path):
            try:
                ...
                self.logger.info(f"💾 기존 파일에서 {len(existing_precedents)}건의 {self.FRIENDLY_NAME} 를 로드했습니다. 재개합니다.")
            except Exception as e:
                self.logger.error(f"❌ 기존 파일 로드 중 오류 발생 ({self.detail_output_path}). 새롭게 시작합니다. 오류: {e}")
                existing_precedents = []
