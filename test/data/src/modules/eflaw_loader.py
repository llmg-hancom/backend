        ...
        try:
            ...
            for doc in tqdm(documents_to_process, desc="Processing Law Documents"):
                try:
                    ...
                    self.logger.debug(f"Document ID {document_id} 삽입 완료. 청크 임베딩 시작.")
                    ...
                    if all_chunks_for_doc:
                        ...
                        self.logger.info(f"Document ID {document_id}에 대해 총 {count_current_doc}개의 청크를 일괄 삽입합니다 (COPY).")
            ...
            self.logger.info(f"=== ETL 파이프라인 최종 요약: {processed_doc_count}개의 문서 처리 완료, 총 {inserted_chunk_count}개의 청크 삽입/갱신 완료. ===")
            cur.close()
