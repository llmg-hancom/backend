...
class BaseLoader(ABC):
    ...
    def _init_embeddings(self) -> OllamaEmbeddings:
        ...
        logger.info(f"Ollama 임베딩 모델 '{self.OLLAMA_MODEL}' 초기화 완료.")
        return embeddings
    ...
    def run_etl_pipeline(self, raw_data_list: List[Dict[str, Any]]):
        ...
        try:
            ...
            logger.info(f"=== ETL 파이프라인 최종 요약: {processed_doc_count}개의 {self.friendly_name} 문서 처리 완료, 총 {inserted_chunk_count}개의 청크 삽입/갱신 완료. ===")
            cur.close()
